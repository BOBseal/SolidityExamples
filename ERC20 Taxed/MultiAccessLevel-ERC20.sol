...
contract Token is IERC20 ,ReentrancyGuard{
    ...
    mapping(address => Admins) private AdminList;
    ...
    function addAdmin(address account , uint256 accessLevel) external {
        require(msg.sender == owner(), "Only the owner can remove an admin");
        require(accessLevel <=3 ,"There Can Only be 3 levels of Access"); //3 = owner , 1 = get access to few functions
        require(isAdmin[account] == false , "Already an Admin");
        isAdmin[account] = true;
        AdminList[account] = Admins(account , accessLevel);
    }
    ...
    function getAdmin(address account) external view returns (Admins memory) {
    require(isAdmin[msg.sender] && AdminList[msg.sender].accessLevel > 0 || msg.sender == owner() ||msg.sender == address(this), "Not an admin");
    return AdminList[account];
    }
    function getAdminAccessLevel(address account) external view returns (uint256) {
    require(isAdmin[msg.sender] && AdminList[msg.sender].accessLevel > 0 || msg.sender == owner() ||msg.sender == address(this), "Not an admin");
    return AdminList[account].accessLevel;
    }
